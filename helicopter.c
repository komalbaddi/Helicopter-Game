...
void display(void)
{
	...
	else if(wflag==1)//Welcome Screen
	{
		...
		glColor3f(1.0,1.0,1.0);
		renderBitmapString(35,85,0,GLUT_BITMAP_HELVETICA_18,"KLEIT ENGINEERING COLLEGE");
		renderBitmapString(41,80,0,GLUT_BITMAP_HELVETICA_12,"HUBLI, Karnataka");
	...
	//within every level make level_flag=1
		...
		scf+=0.005; //so less as program run very fast
		sci=(int)scf;
		sprintf(scs,"%d",sci);
